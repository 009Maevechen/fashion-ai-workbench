...
const project={profile:{attributes:{fabric:"针织",fabricTexture:"罗纹"}},targetColors:[{id:"brown",name:"深卡其色",outputName:"深卡其色黑色扣子",hex:"#8B7355",trimColorName:"黑边",trimHex:"#101010",structureMode:"same_style",styleRelation:"same"}]} as Project;
...
test("复色一致性以第一张锁款式、第二张提供颜色映射",()=>{
  ...
  assert.match(prompt,/第1张已确认姿势图是人物、姿势、动作、景别、构图、背景、画面样式、服装版型、结构、面料、纹理、垂感、扣子五金和全部颜色区域布局的唯一底图/);
  assert.match(prompt,/第2张当前颜色款参考图只负责提供普通颜色款的主体色/);
  assert.match(prompt,/当前颜色款没有获准改变结构/);
  assert.match(prompt,/扣子、五金和其他细节是否保持第1张不变/);
