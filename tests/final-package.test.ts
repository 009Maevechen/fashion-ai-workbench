...
test("上游过期和尚未人工确认的复色图会明确提示",()=>{
  assert.ok(finalPackageIssues({...project,dependencyStatus:"needs_review"}).length>0);
  assert.ok(finalPackageIssues({...project,targetColors:[{...project.targetColors![0],status:"success",poseResults:["/one.jpg"]}]}).some(issue=>issue.includes("还没有生成出可导出照片")));
});
...
test("错误和未完成项只作提示，只要有照片就可组成导出包",()=>{
  ...
  assert.equal(finalPackagePhotoCount(partial),3);
...
test("最终照片只统计逐张人工确认的复色图",()=>{
  const partial={...project,targetColors:[{...project.targetColors![0],status:"partial_success" as const,poseResults:["/generated/one.jpg","/generated/two.jpg"],confirmedPoseResults:["/confirmed/one.jpg",""]}]};
  assert.equal(finalPackagePhotoCount(partial),4);
  assert.ok(finalPackageIssues(partial).some(issue=>issue.includes("还没有生成出可导出照片"))===false);
});
