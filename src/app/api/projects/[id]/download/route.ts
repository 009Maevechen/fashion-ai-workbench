...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  const p=await getProject((await params).id);if(!p)return NextResponse.json({error:"项目不存在"},{status:404});const colors=recolorColorsWithSavedJobs(p,await listJobs({projectId:p.id})),query=new URL(request.url).searchParams,requestedWorkflow=query.get("workflow"),workflow=["tryon","pose","recolor"].includes(requestedWorkflow||"")?requestedWorkflow:undefined;
  ...
  if(!workflow||workflow==="recolor"){for(const [colorIndex,color] of colors.entries()){const colorName=safeSegment(color.name)||`颜色${colorIndex+1}`;for(const [i,u] of (color.poseResults||[]).entries())if(await add(archive,used,`recolor/${colorName}/${sku}_${colorName}_pose${String(i+1).padStart(2,"0")}.jpg`,u))count++}const known=new Set(colors.flatMap(color=>color.poseResults||[]));for(const [i,u] of (p.confirmedRecolorImages||[]).filter(url=>!known.has(url)).entries())if(await add(archive,used,`recolor/未分类/${sku}_recolor_${String(i+1).padStart(2,"0")}.jpg`,u))count++}
  ...
  if(!workflow){
    ...
    for(const [colorIndex,color] of colors.entries()){
      ...
      for(const [i,u] of (color.poseResults||[]).entries())if(await add(archive,used,`${sku}/${colorName}/${sku}_${colorName}_姿势${String(i+1).padStart(2,"0")}.jpg`,u))count++;
    }
    const grouped=new Set(colors.flatMap(color=>color.poseResults||[]));
