...
export async function DELETE(_:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const cleanup=prepareSourceAssetCleanup(project);if(!cleanup.clearedAssetCount&&!cleanup.urls.length)return NextResponse.json({project,message:"当前任务没有可清空的素材",clearedCount:0});
    await moveFilesToTrashStrict(cleanup.urls,id);
    await updateProject(id,{assets:cleanup.assets,assetEvidence:{},productVisualAnalysis:undefined,productImageEnhancement:undefined,targetColors:cleanup.targetColors,poseReferenceInputs:[],selectedPoseTemplateGroupId:undefined,poseTemplateSnapshot:undefined,garmentDetailLock:undefined});
    ...
    return NextResponse.json({project:await getProject(id),message:"当前任务素材已清空，生成结果已保留",clearedCount:cleanup.clearedAssetCount});
