...
export async function POST(request:Request){
  try{
    ...
    if(projectId&&KEYS.has(assetKey)){
      ...
      if(ARRAY_KEYS.has(assetKey)){const max=assetKey==="standaloneRecolorPoseImages"?2:4;if(index<0||index>max)throw new Error("素材图片序号无效");const values=[...((assets[assetKey] as string[]|undefined)||[])];values[index]=url;Object.assign(assets,{[assetKey]:values})}
      else Object.assign(assets,{[assetKey]:url});
