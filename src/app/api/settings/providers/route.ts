...
const schema=z.object({name:z.string().min(1).max(80),type:z.enum(["openai-compatible","fashn","bfl","volcengine","flux","custom"]),baseUrl:z.string().min(1).max(500),apiKey:z.string().min(1).max(1000),defaultModel:z.string().min(1).max(200),enabled:z.boolean().optional(),notes:z.string().max(500).optional()});
