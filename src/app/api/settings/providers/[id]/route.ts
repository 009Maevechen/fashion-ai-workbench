...
const schema=z.object({name:z.string().min(1).max(80).optional(),type:z.enum(["openai-compatible","fashn","bfl","volcengine","flux","custom","deepseek"]).optional(),baseUrl:z.string().min(1).max(500).optional(),apiKey:z.string().max(1000).optional(),defaultModel:z.string().min(1).max(200).optional(),enabled:z.boolean().optional(),notes:z.string().max(500).optional()});
