...
import {readImageDerivative,readOutput} from "@/lib/ai/storage";
...
export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  try{const parts=(await params).path,query=new URL(request.url).searchParams,thumb=Number(query.get("thumbnail")||0),preview=Number(query.get("preview")||0),kind=thumb?"thumbnail":preview?"preview":undefined,requested=thumb||preview,b=kind?await serializeImageWork(()=>readImageDerivative(parts,kind,requested)):await readOutput(parts);return new NextResponse(new Uint8Array(b),{headers:{...headers(parts,b.byteLength),"Content-Type":requested?"image/jpeg":mimeFor(parts),"Cache-Control":requested?"private, max-age=86400, immutable":"private, max-age=3600"}})}
