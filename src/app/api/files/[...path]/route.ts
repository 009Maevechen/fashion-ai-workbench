...
export async function GET(request:Request,{params}:{params:Promise<{path:string[]}>}){
  try{const parts=(await params).path,original=await readOutput(parts),requested=Number(new URL(request.url).searchParams.get("thumbnail")||0),b=requested?await sharp(original,{limitInputPixels:80_000_000}).rotate().resize({width:Math.max(120,Math.min(960,requested)),height:1280,fit:"inside",withoutEnlargement:true}).jpeg({quality:78,mozjpeg:true}).toBuffer():original;return new NextResponse(new Uint8Array(b),{headers:{...headers(parts,b.byteLength),"Content-Type":requested?"image/jpeg":mimeFor(parts),"Cache-Control":requested?"private, max-age=86400, immutable":"private, max-age=3600"}})}
