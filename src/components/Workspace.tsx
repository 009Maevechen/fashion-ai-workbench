...
export default function Workspace({initial,initialJobs,health,modelRouting,initialStep}:{initial:Project;initialJobs:Job[];health:ApiHealth;modelRouting:WorkflowRuntimeSummary;initialStep?:number}){
  ...
  const [p,setP]=useState(initial),[step,setStep]=useState(Math.min(Math.max(requested,1),5)),[jobs,setJobs]=useState(initialJobs),[busy,setBusy]=useState(false),[error,setError]=useState(""),[notice,setNotice]=useState("");
  ...
  async function post(url:string,body:unknown){const response=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}),data=await response.json();if(!response.ok)throw new Error(data.error);if(!data.jobId)return data;for(let attempt=0;attempt<180;attempt++){const poll=await fetch(`/api/jobs/${data.jobId}`,{cache:"no-store"}),state=await poll.json();if(!poll.ok)throw new Error(state.error||"任务状态查询失败");await refresh();if(state.status==="success")return state;if(state.status==="failed"||state.status==="interrupted")throw new Error(state.error||"生成任务失败");await new Promise(resolve=>setTimeout(resolve,1000))}throw new Error("本地任务等待超时，请在历史任务中查看并重试")}
  const run:Runner=fn=>{setBusy(true);setError("");void fn().then(refresh).catch(e=>setError(e instanceof Error?e.message:"操作失败")).finally(()=>setBusy(false))};
  ...
  const common:Omit<PanelProps,"jobs">={p,health,modelRouting,busy,run,persistAsset,deleteAsset,clearSourceAssets,clearWorkflowResults,saveProject,post,confirmFlow};
