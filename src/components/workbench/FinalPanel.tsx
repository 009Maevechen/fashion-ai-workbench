...
export default function FinalPanel({p,jobs,onStep,onComplete}:{p:Project;jobs:Job[];onStep:(step:number)=>void;onComplete:()=>Promise<void>}){
  ...
  const colors=p.targetColors||[],confirmedColors=colors.filter(isColorSetComplete),duplicates=duplicateColorNames(colors),incompleteColors=colors.filter(color=>!isColorSetComplete(color));
  ...
  const blockers=[!p.confirmedTryonImage&&"尚未确认换装结果",p.confirmedPoseImages?.length!==3&&"尚未确认三张姿势图",colors.length===0&&"尚未建立任何颜色套装",incompleteColors.length>0&&`${incompleteColors.length} 套颜色尚未命名、未生成完整三张或未确认`,duplicates.size>0&&"存在重复颜色名称",p.dependencyStatus&&p.dependencyStatus!=="current"&&"上游输入已经变化，当前结果需要重新生成或重新审核",staleColors.length>0&&`${staleColors.length} 套复色结果已过期`].filter(Boolean) as string[];
