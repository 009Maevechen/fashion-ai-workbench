...
export default function RecolorPanel({p,jobs,health,modelRouting,busy,run,refreshProject,persistAsset,deleteAsset,clearSourceAssets,clearWorkflowResults,saveProject,post}:PanelProps){
  ...
  const resultColors=colors.filter(color=>Boolean(color.poseResults?.length)||currentJobsFor(color).length>0||color.status==="generating");
  ...
  return <>
    ...
    <div className="recolor-production-grid">
      <section className="card recolor-results-card">
        <div className="panel-head"><div><h2>{active?`${activeName||"未命名颜色"} · 生成结果`:"复色生成结果"}</h2><small>勾选一张或多张图片，可在右侧统一重新生成</small></div><div className="panel-actions recolor-retry-toolbar"><button type="button" className="secondary" disabled={!active||!colorJobs.some(job=>job.status==="failed"||job.status==="interrupted")} onClick={()=>setRetrySlots(Array.from({length:expectedCount},(_,index)=>index+1).filter(slot=>{const job=colorJobs.find(item=>item.slot===slot);return job?.status==="failed"||job?.status==="interrupted"}))}>选择失败图片</button><button type="button" className="primary" disabled={busy||retrySlots.length===0} onClick={()=>run(retrySelected)}>重新生成选中图片{retrySlots.length?`（${retrySlots.length}）`:""}</button><a className="button secondary" href={`/api/projects/${p.id}/download`}>下载全部图片 ZIP</a><span className="badge">{active?.status||"未建立颜色"}</span><ClearResultsButton workflow="recolor" disabled={busy||!hasWorkflowResults(p,jobs,"recolor")} onConfirm={clearResults}/></div></div>
        {resultColors.length>0&&<div className="recolor-result-tabs">{resultColors.map(color=>{const liveJobs=currentJobsFor(color),liveCount=new Set(liveJobs.flatMap(job=>job.outputImages)).size,done=color.status==="generating"?liveCount:Math.max(colorResultCount(color),liveCount),required=expectedColorResultCount(color),running=color.status==="generating"||liveJobs.some(job=>["queued","submitting","waiting_provider","downloading","validating","saving","generating"].includes(job.status));return <button type="button" className={color.id===activeId?"active":""} key={color.id} onClick={()=>setActiveId(color.id)}><i style={{background:VALID_HEX.test(color.hex||"")?color.hex:"#E8E8EE"}}/><span>{normalizedColorName(color)||"未命名颜色"}</span><small>{running?"生成中":`${done}/${required}张`}</small></button>})}</div>}
        {active?<div className={`result-grid ${expectedCount===3?"three":expectedCount===4?"four":""}`}>{Array.from({length:expectedCount},(_,index)=>index+1).map(slot=>{const job=colorJobs.find(item=>item.slot===slot),url=job?.outputImages[0],selected=retrySlots.includes(slot);return <div className={`recolor-result-pair selectable ${selected?"selected":""}`} key={slot}><label className="recolor-result-select"><input type="checkbox" checked={selected} onChange={()=>setRetrySlots(current=>selected?current.filter(item=>item!==slot):[...current,slot])}/><span>{selected?"已选择":"选择图片"}</span></label><ResultCard job={job} pending={active.status==="generating"} label={`复色后 · 姿势${slot}`} onPreview={url?()=>setPreview({images:collectionImages,index:Math.max(0,collectionImages.indexOf(url))}):undefined} onRetry={()=>run(()=>start(slot))} onFallbackRetry={route.fallback.configured?()=>run(()=>start(slot,"fallback")):undefined}/></div>})}</div>:<div className="empty-state">请先识别或添加颜色</div>}
