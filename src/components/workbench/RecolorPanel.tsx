...
export default function RecolorPanel({p,jobs,historyJobs,health,modelRouting,busy,run,refreshProject,persistAsset,deleteAsset,clearSourceAssets,clearWorkflowResults,cancelGeneration,clearWorkflowErrors,saveProject,post,enqueue}:PanelProps&{historyJobs:Job[]}){
  ...
  async function analyzeReference(){
    setAnalyzing(true);
    try{
      const response=await fetch(`/api/projects/${p.id}/colors/analyze`,{method:"POST"}),data=await response.json() as {colors?:Array<{name:string;generationName?:string;hex:string;trimColorName?:string;trimHex?:string;confidence?:number;designDetails?:string[];materialFeatures?:string;colorRegions?:TargetColor["colorRegions"];isUniformColor?:boolean;uniformColorConfidence?:number;occlusion?:TargetColor["occlusion"];occlusionPolicy?:TargetColor["occlusionPolicy"];occlusionReason?:string;needsReview?:boolean;reviewReason?:string;cropImage?:string;cropRegion?:CropRegion}>;needsReview?:boolean;reviewReason?:string;confidence?:number;error?:string};
      if(!response.ok||!data.colors)throw new Error(data.error||"颜色分析失败");
      // 重新识别颜色时清空旧的颜色任务队列，从零重新计算，不再与旧色卡合并。
      const next=data.colors.map(color=>({id:crypto.randomUUID(),name:color.name,outputName:analyzedColorName(color),baseHex:color.hex,hex:color.hex,trimColorName:color.trimColorName,trimHex:color.trimHex,designDetails:color.designDetails,materialFeatures:color.materialFeatures,colorRegions:color.colorRegions,isUniformColor:color.isUniformColor,uniformColorConfidence:color.uniformColorConfidence,occlusion:color.occlusion,occlusionPolicy:color.occlusionPolicy,occlusionReason:color.occlusionReason,designConfidence:color.confidence,designNeedsReview:Boolean(color.needsReview||(color.confidence??1)<0.65||!color.cropImage),manualReviewConfirmed:false,cropImage:color.cropImage,cropRegion:color.cropRegion,status:color.cropImage?"ready" as const:"draft" as const}));
  ...
  async function crop(region:CropRegion){if(!active)return;const result=await post(`/api/projects/${p.id}/colors/crop`,{colorId:active.id,region}) as {url:string};await updateColor({cropRegion:region,cropImage:result.url,status:"ready",manualReviewConfirmed:true,designNeedsReview:false})}
  ...
  async function toggleColorLock(){
    if(!colorsLocked){
      ...
      const pendingReview=colors.filter(color=>color.designNeedsReview&&!color.manualReviewConfirmed);
      if(pendingReview.length)throw new Error(`还有 ${pendingReview.length} 个颜色款存在遮挡或低置信度信息，请逐款点击“人工确认参考规则”`);
    }
  ...
  const colorControlPanel=<section className={`card recolor-control-card ${paletteVisible?"palette-open":""}`}>
    ...
    {active&&paletteVisible?<><ColorAdjustmentPanel key={active.id} baseHex={active.baseHex||active.hex} adjustment={active.colorAdjustment} disabled={busy} onApply={applyColorAdjustment}/>{reference.url&&<details className="recolor-crop-details"><summary>{active.cropImage?"重新框选该颜色款整件服装":"框选该颜色款整件服装（必需）"}</summary><div className="notice">请完整包含该色款可见的领口、袖口、口袋、拼接、下摆或裤脚；不能只框一个颜色小块。人工重新框选视为确认当前可见参考区域。</div><ColorCropper src={referenceSourceUrl||reference.url} region={active.cropRegion} onChange={region=>run(()=>crop(region))}/></details>}</>:<div className="notice">点击识别出的色块或“调整颜色”，这里才会显示色板。</div>}
    ...
    <div className="generate-footer"><button className="primary" disabled={busy||!active||!activeName||activeNameDuplicate||sources.length<2||sources.length>4||!hasTargetColor(active)||!configured||Boolean(active?.designNeedsReview&&!active.manualReviewConfirmed)} onClick={()=>run(()=>start())}>单独重试{activeName||"当前颜色"}（{sources.length>=2?sources.length:"3–4"}张）</button></div>
  </section>;
    ...
    <section className="card recolor-flow-card">
      ...
      <div className="recolor-source-reference-grid">
        <div className="recolor-reference-column">
          ...
          <div className="recolor-mode-toggle"><div><h3>逐款参考图复色</h3><small>已强制启用一款一图、一款一套颜色与设计规则</small></div><small className="setting-help">每一款都使用自己的整件服装参考图。名称、标题和 HEX 仅辅助；与图片冲突时始终以参考图真实可见内容为准。</small></div>
          <div className="recolor-reference-note"><b>识别原则</b><span>自动识别每个颜色款，并尝试为每款裁出整件服装设计参考；裁图必须包含口袋、条纹、拼接、扣子、印花、包边和面料分区。遮挡款只有在明确为统一单色时才会延展可见颜色，否则必须人工确认。</span></div>
        </div>
        ...
        <div className="recolor-sampler-left">
          ...
          <section className="card recolor-analysis-card">
            ...
            {colors.length?<div className="recolor-color-editor-list">{colors.map((color,index)=>{const count=colorResultCount(color),issue=count?`已收集 ${count} 张`:colorSetIssue(color,duplicateNames),required=expectedColorResultCount(color),checked=batchColorIds.includes(color.id);return <div className={`recolor-color-editor ${color.id===activeId?"active":""}`} key={color.id}>
              ...
              <div className="recolor-color-fields"><label>主体颜色<input disabled={colorsLocked} defaultValue={color.name} placeholder={`颜色 ${index+1}`} onBlur={event=>run(()=>updateColorById(color.id,{name:event.target.value}))}/></label><label>主色HEX<input disabled={colorsLocked} defaultValue={color.hex||""} placeholder="#C8A06A" onBlur={event=>run(()=>updateColorById(color.id,{baseHex:event.target.value.toUpperCase(),hex:event.target.value.toUpperCase()}))}/></label><label>边饰/局部颜色<input disabled={colorsLocked} defaultValue={color.trimColorName||""} placeholder="例如：白色 / 黑色" onBlur={event=>run(()=>updateColorById(color.id,{trimColorName:event.target.value,outputName:undefined}))}/></label><label>边饰HEX<input disabled={colorsLocked} defaultValue={color.trimHex||""} placeholder="可选" onBlur={event=>run(()=>updateColorById(color.id,{trimHex:event.target.value.toUpperCase()}))}/></label><label className="recolor-output-name">颜色名称（仅辅助命名）<input key={`${color.id}-${recolorColorName(color)}`} disabled={colorsLocked} defaultValue={recolorColorName(color)} placeholder="例如：白色黑边 / 黑色白条纹" onBlur={event=>run(()=>{const outputName=event.target.value.trim(),trim=recolorGenerationTrim({...color,outputName});return updateColorById(color.id,{outputName,...trim})})}/></label></div>
              <div className="recolor-color-status"><b>{colorResultCount(color)}/{required} 张</b><small>{issue}</small>{(()=>{const semantics=parseColorName(recolorColorName(color));return <>{semantics.parts.length>0&&<small className="recolor-semantics">名称辅助：{semantics.parts.map(p=>`${p.part}：${p.color}`).join("、")}</small>}{semantics.needsReview&&<small className="recolor-semantics warn">⚠ 名称不参与覆盖图片，请按参考图确认</small>}</>})()}<small title={color.colorRegions?.map(region=>`${region.part}：${region.colorName}`).join("、")}>{color.colorRegions?.length?`可见配色：${color.colorRegions.slice(0,4).map(region=>`${region.part}${region.colorName}`).join("、")}`:"可见配色：以参考图为准"}</small><small title={color.designDetails?.join("、")}>{color.designDetails?.length?`设计：${color.designDetails.slice(0,3).join("、")}`:"设计：等待识别或人工框选"}</small>{color.occlusion&&color.occlusion!=="none"&&<small className={color.occlusionPolicy==="extend_uniform"?"recolor-semantics":"recolor-semantics warn"}>{color.occlusionPolicy==="extend_uniform"?"遮挡：已确认统一单色，可延展可见颜色":"⚠ 遮挡：不可猜色，需要人工确认"}</small>}{color.materialFeatures&&<small title={color.materialFeatures}>面料：{color.materialFeatures}</small>}</div>
              {color.designNeedsReview&&!color.manualReviewConfirmed&&<button type="button" className="secondary" disabled={colorsLocked} onClick={()=>run(()=>updateColorById(color.id,{manualReviewConfirmed:true}))}>人工确认参考规则</button>}
              {color.manualReviewConfirmed&&<span className="badge success">已人工确认</span>}
            ...
            <div className="notice">复色硬规则：每个色款必须拥有自己的整件服装设计参考；参考图真实可见内容优先于名称、标题和 HEX。同一组三姿势逐款复刻主体色、局部配色、口袋、条纹、拼接、扣子、印花、包边、线条位置和面料分区。非统一色或无法判断的遮挡区域禁止猜测，必须人工确认。</div>
            <div className="recolor-batch-bar"><div><b>已选择 {selectedBatchColors.length} 款颜色</b><span>{colorsLocked?"颜色已锁定":"请确认参考图后锁定"} · 预计生成 {selectedBatchColors.length*(sources.length>=2?sources.length:0)} 张图片</span></div><div className="recolor-resolution"><span>输出尺寸</span>{[["fast","1K"],["standard","2K"],["quality","4K"]].map(([key,label])=><button type="button" className={mode===key?"active":""} key={key} onClick={()=>setMode(key as typeof mode)}>{label}</button>)}</div><button type="button" className={`recolor-lock-button ${colorsLocked?"locked":""}`} disabled={busy||analyzing} onClick={()=>run(toggleColorLock)}>{colorsLocked?"解除锁定":"锁定颜色"}</button><button className="primary" disabled={busy||!colorsLocked||!configured||sources.length<2||sources.length>4||selectedBatchColors.length===0||duplicateNames.size>0||selectedBatchColors.some(color=>!normalizedColorName(color)||!hasTargetColor(color)||Boolean(color.designNeedsReview&&!color.manualReviewConfirmed))} onClick={()=>run(startBatch)}>开始批量复色</button><GenerationControls workflow="recolor" jobs={historyJobs} cancelGeneration={cancelGeneration} clearWorkflowErrors={clearWorkflowErrors} run={run}/></div>
          </section>
        </div>
        {colorControlPanel}
      </div>}
