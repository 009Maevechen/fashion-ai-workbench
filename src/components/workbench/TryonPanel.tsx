...
</div>{jobs.length?<div className="result-grid tryon-single-result"><div><ResultCard key={`tryon-candidate-slot-${activeCandidate}`} job={visibleJob} label={`候选 ${String(activeCandidate).padStart(2,"0")}`} selected={selected===visibleUrl} onSelect={visibleUrl?()=>setSelected(visibleUrl):undefined} onPreview={visibleUrl?()=>setPreview({images:allImages,index:allImages.indexOf(visibleUrl)}):undefined} onRetry={()=>run(()=>start(activeCandidate))} onFallbackRetry={route.fallback.configured?()=>run(()=>start(activeCandidate,"fallback")):undefined} onCorrect={visibleJob?request=>run(()=>post(`/api/jobs/${visibleJob.id}/retry`,{correctionRequest:request})):undefined} correctionBusy={busy}/><ConsistencyCheck job={visibleJob} busy={busy} onCheck={()=>visibleJob&&run(()=>checkConsistency(visibleJob.id))}/></div></div>:<div className="empty-state">
<div>
