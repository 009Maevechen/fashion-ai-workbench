...
export default function TryonPanel({p,jobs,historyJobs,health,modelRouting,busy,run,persistAsset,deleteAsset,clearSourceAssets,clearWorkflowResults,cancelGeneration,clearWorkflowErrors,saveProject,post,enqueue,confirmFlow}:PanelProps&{historyJobs:Job[]}){
  ...
  const selectedJob=historyJobs.find(job=>job.outputImages.includes(selected));
  const confirmHint=!selected?"请先在上方候选图中点击「选择此结果」":!selectedJob?"所选结果已不在当前任务中，请重新生成":selectedJob.status==="failed"?"该结果未通过换装主体校验或生成失败，请重新生成":selectedJob.status==="interrupted"?"该结果生成被中断，请重新生成":selectedJob.status==="stale"||selectedJob.dependencyStatus==="stale"?"该结果已过期，请重新生成":"确认后可进入三种姿势";
