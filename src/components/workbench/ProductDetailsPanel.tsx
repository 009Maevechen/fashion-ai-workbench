...
export default function ProductDetailsPanel({p,jobs,busy,run,persistAsset,deleteAsset,clearSourceAssets,saveProject,onNext}:{p:Project;jobs:Job[];busy:boolean;run:Runner;persistAsset:(file:File|undefined,key:keyof ProjectAssets,name:string,index?:number)=>Promise<string>;deleteAsset:(key:keyof ProjectAssets,index?:number)=>Promise<void>;clearSourceAssets:()=>Promise<Project>;saveProject:(patch:Partial<Project>)=>Promise<Project>;onNext:()=>void}){
  ...
  const [tagInput,setTagInput]=useState(""),[saved,setSaved]=useState(true),[preview,setPreview]=useState<string|null>(null),[activeTab,setActiveTab]=useState<ProductTab>("basic");
  ...
  return <div className="product-details-page">
    <nav className="product-tabs" aria-label="商品资料分区" role="tablist">{PRODUCT_TABS.map(tab=><button type="button" role="tab" aria-selected={activeTab===tab.id} className={activeTab===tab.id?"active":""} key={tab.id} onClick={()=>setActiveTab(tab.id)}><span>{tab.label}</span><small>{tab.description}</small></button>)}</nav>
    {!saved&&<div className="product-unsaved">商品资料有未保存修改，请完成后点击保存。</div>}
    {activeTab==="basic"&&<div className="product-tab-panel product-basic-view" role="tabpanel">
      <section className="card"><div className="panel-head"><h2>基础信息</h2><span className={`badge ${profile.reviewStatus==="confirmed"?"success":"wait"}`}>{profile.reviewStatus==="confirmed"?"已确认":profile.reviewStatus==="awaiting_review"?"等待人工确认":"草稿"}</span></div>
    ...
    {activeTab==="attributes"&&<div className="product-tab-panel" role="tabpanel"><section className="card"><div className="panel-head"><h2>商品属性与服装结构</h2><small>保存后自动进入换装提示词和人工复核</small></div><div className="attribute-grid">{(Object.keys(ATTRIBUTE_LABELS) as (keyof ProductAttributes)[]).map(key=><label className="field" key={key}>{ATTRIBUTE_LABELS[key]}<select value={profile.attributes?.[key]||""} onChange={e=>changeAttribute(key,e.target.value)}><option value="">请选择</option>{ATTRIBUTE_OPTIONS[key].map(item=><option key={item}>{item}</option>)}</select></label>)}</div><h3 className="section-label">重点保护</h3><div className="protection-grid">{DEFAULT_PROTECTION_ITEMS.map(item=><label className="check-item" key={item}><input type="checkbox" checked={(profile.protectionItems||[]).includes(item)} onChange={()=>changeProfile({protectionItems:(profile.protectionItems||[]).includes(item)?(profile.protectionItems||[]).filter(value=>value!==item):[...(profile.protectionItems||[]),item]})}/>{item}</label>)}</div></section></div>}
    {activeTab==="assets"&&<div className="product-tab-panel" role="tabpanel"><section className="card product-assets"><div className="panel-head"><h2>图片素材</h2><div className="panel-actions"><span>{previewImages.length} 张已保存</span><ClearAssetsButton disabled={busy||!hasClearableSourceAssets(p)} onConfirm={clearAllAssets}/></div></div><div className="product-assets-grid">{ASSET_FIELDS.map(field=><div className="product-asset-card" key={field.key}><AssetUploadCard label={`${field.label}${field.required?" *":""}`} description={field.description} value={assets[field.key]||{status:"idle"}} onChange={asset=>run(()=>saveAsset(asset,field.key,field.name))} onDelete={assets[field.key]?.url?()=>run(()=>removeAsset(field.key)):undefined} onPreview={()=>assets[field.key]?.url&&setPreview(assets[field.key].url!)}/></div>)}<div className="product-asset-card"><AssetUploadCard label="其他素材" description="可补充包装、配饰或说明图" value={assets.otherMaterial||{status:"idle"}} onChange={asset=>run(()=>saveAsset(asset,"otherMaterialImages","other-material-01",0))} onDelete={assets.otherMaterial?.url?()=>run(()=>removeAsset("otherMaterialImages",0)):undefined} onPreview={()=>assets.otherMaterial?.url&&setPreview(assets.otherMaterial.url!)}/></div></div><p className="product-hint">平铺图和模特参考图用于换装；细节图与面料图用于帮助模型保护商品结构和纹理。</p></section></div>}
    {activeTab==="description"&&<div className="product-tab-panel product-narrow-panel" role="tabpanel"><section className="card"><div className="panel-head"><h2>细节要求</h2><small>给生成模型的重点保护提示</small></div><label className="field"><textarea className="product-detail-textarea" maxLength={800} value={profile.detailDescription||""} onChange={e=>changeProfile({detailDescription:e.target.value})} placeholder="描述服装颜色、版型、领口、袖口、下摆、纽扣、印花、包边和面料等需要严格保持的细节。"/><span className="field-count">{profile.detailDescription?.length||0}/800</span></label><div className="suggestion-box"><b>建议写法</b><p>按“服装类别 → 版型与长度 → 领口袖口 → 纽扣印花 → 包边与面料”的顺序描述，模型更容易准确理解。</p></div></section></div>}
    {activeTab==="tags"&&<div className="product-tab-panel product-tags-view" role="tabpanel"><section className="card"><div className="panel-head"><h2>标签与备注</h2><small>方便检索、交接和人工复核</small></div><h3 className="section-label">标签管理</h3><div className="tag-list">{profile.tags?.map(tag=><button type="button" key={tag} onClick={()=>changeProfile({tags:profile.tags?.filter(item=>item!==tag)})}>{tag} ×</button>)}</div><div className="tag-add"><input value={tagInput} onChange={e=>setTagInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();addTag()}}} placeholder="输入标签"/><button type="button" className="secondary" onClick={addTag}>添加</button></div><h3 className="section-label">项目备注</h3><label className="field"><textarea maxLength={300} value={profile.notes||""} onChange={e=>changeProfile({notes:e.target.value})} placeholder="填写交接事项、拍摄要求或审核说明"/><span className="field-count">{profile.notes?.length||0}/300</span></label></section><section className="card product-stats"><div className="panel-head"><h2>任务统计</h2><small>真实任务数据</small></div><dl><div><dt>历史生成任务</dt><dd>{jobs.length}</dd></div><div><dt>成功／待审核</dt><dd>{successful}</dd></div><div><dt>创建时间</dt><dd>{new Date(p.createdAt).toLocaleString("zh-CN")}</dd></div><div><dt>最后更新</dt><dd>{new Date(p.updatedAt).toLocaleString("zh-CN")}</dd></div></dl></section></div>}
    <div className="product-save-bar"><div><b>{PRODUCT_TABS.find(tab=>tab.id===activeTab)?.label}</b><span>{saved?"当前修改已保存":"当前页面有未保存修改"}</span></div><button className="secondary" type="button" disabled={busy||saved} onClick={()=>run(()=>persist(false))}>保存资料</button><button className="primary" type="button" disabled={busy||!sku.trim()||!name.trim()||!assets.garmentImage?.url} onClick={()=>run(()=>persist(true))}>保存并进入换装</button></div>
