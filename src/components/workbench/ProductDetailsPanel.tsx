...
const ATTRIBUTE_LABELS:Record<keyof ProductAttributes,string>={mainColor:"主色",printType:"印花类型",neckline:"领口类型",sleeveType:"袖型",garmentLength:"衣长",fit:"版型",fabric:"面料",transparency:"透明度",lining:"是否有内衬",elasticity:"弹性"};
type SingleAssetKey="garmentImage"|"productDetailImage"|"brandTagImage"|"modelReferenceImage"|"fabricTextureImage";
const ASSET_FIELDS:{key:SingleAssetKey;label:string;description:string;name:string;required?:boolean}[]=[
  {key:"garmentImage",label:"服装平铺图",description:"用于换装的主商品图",name:"garment",required:true},
  {key:"productDetailImage",label:"服装细节图",description:"领口、袖口、印花等细节",name:"product-detail"},
  {key:"brandTagImage",label:"品牌／标签图",description:"吊牌、水洗标或包装标签",name:"brand-tag"},
  {key:"modelReferenceImage",label:"模特参考图",description:"保留模特、姿势和背景",name:"model-reference",required:true},
  {key:"fabricTextureImage",label:"面料纹理图",description:"近距离面料和纹理参考",name:"fabric-texture"},
];
...
export default function ProductDetailsPanel({p,jobs,busy,run,persistAsset,deleteAsset,clearSourceAssets,saveProject,onNext}:{p:Project;jobs:Job[];busy:boolean;run:Runner;persistAsset:(file:File|undefined,key:keyof ProjectAssets,name:string,index?:number)=>Promise<string>;deleteAsset:(key:keyof ProjectAssets,index?:number)=>Promise<void>;clearSourceAssets:()=>Promise<Project>;saveProject:(patch:Partial<Project>)=>Promise<Project>;onNext:()=>void}){
  ...
  const [profile,setProfile]=useState<ProductProfile>({...initialProfile,reviewStatus:initialProfile.reviewStatus||"draft",tags:initialProfile.tags||[],attributes:initialProfile.attributes||{}});
  ...
  const checks=useMemo(()=>[
    ...
    {label:"模特参考图完整",ok:Boolean(assets.modelReferenceImage?.url)},
  ...
  return <div className="product-details-page">
    ...
    <div className="product-details-top">
      <section className="card" id="product-basic"><div className="panel-head"><h2>基础信息</h2><span className={`badge ${profile.reviewStatus==="confirmed"?"success":"wait"}`}>{profile.reviewStatus==="confirmed"?"已确认":profile.reviewStatus==="awaiting_review"?"等待人工确认":"草稿"}</span></div>
        ...
        <div className="product-save-actions"><button className="secondary" type="button" disabled={busy||saved} onClick={()=>run(()=>persist(false))}>保存资料</button><button className="primary" type="button" disabled={busy||!sku.trim()||!name.trim()||!assets.garmentImage?.url||!assets.modelReferenceImage?.url} onClick={()=>run(()=>persist(true))}>保存并进入换装</button></div>
      </section>
    ...
    <div className="product-details-bottom">
      <section className="card" id="product-attributes"><div className="panel-head"><h2>关键属性</h2><small>用于提示词和人工复核</small></div><div className="attribute-grid">{(Object.keys(ATTRIBUTE_LABELS) as (keyof ProductAttributes)[]).map(key=><label className="field" key={key}>{ATTRIBUTE_LABELS[key]}<select value={profile.attributes?.[key]||""} onChange={e=>changeAttribute(key,e.target.value)}><option value="">请选择</option>{ATTRIBUTE_OPTIONS[key].map(item=><option key={item}>{item}</option>)}</select></label>)}</div></section>
