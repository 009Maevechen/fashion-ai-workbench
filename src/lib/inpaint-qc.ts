...
export async function checkInpaintProtectedRegion(source:Buffer,result:Buffer,mask:Buffer):Promise<InpaintBoundaryCheck>{
  const size={width:384,height:512};
  ...
  for(let pixel=0;pixel<size.width*size.height;pixel++){
    ...
    protectedPixels++;totalDelta+=delta;if(delta>20)changed++;
  }
  ...
  if(protectedPixelMeanDelta>8||protectedChangedRatio>0.06)issues.push(`Mask外区域发生明显变化（平均差异 ${protectedPixelMeanDelta.toFixed(1)}，变化像素 ${(protectedChangedRatio*100).toFixed(1)}%）`);
