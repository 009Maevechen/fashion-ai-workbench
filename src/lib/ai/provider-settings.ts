...
import type {ApiProviderPublic,ApiProviderSecretRecord,ApiProviderType,ModelSlot,ProviderRuntimeConfig,ProviderTestStatus,SycConfigInput,SycConfigPublic,SycTestStatus,WorkflowModelBindings,WorkflowModelSelection,WorkflowRuntimeModel,WorkflowRuntimeSummary} from "./provider-settings-types";
...
function publicProvider(record:ApiProviderSecretRecord):ApiProviderPublic{return {id:record.id,name:record.name,type:record.type,baseUrl:record.baseUrl,apiKeyMasked:record.apiKeyMasked,hasApiKey:Boolean(record.encryptedApiKey),defaultModel:record.defaultModel,enabled:record.enabled,notes:record.notes,lastTestStatus:record.lastTestStatus,lastTestAt:record.lastTestAt,lastError:record.lastError,createdAt:record.createdAt,updatedAt:record.updatedAt,imageModel:record.imageModel,chatModel:record.chatModel,stream:record.stream,partialImages:record.partialImages,returnBase64:record.returnBase64,codexCliCompatible:record.codexCliCompatible,timeoutSeconds:record.timeoutSeconds,lastImageTestStatus:record.lastImageTestStatus,lastImageTestAt:record.lastImageTestAt,lastImageTestError:record.lastImageTestError,lastTestLatencyMs:record.lastTestLatencyMs}}
...
export async function updateProviderTestResult(id:string,kind:"connection"|"image",status:ProviderTestStatus,error?:string){return mutate(store=>{const record=store.apiProviders.find(item=>item.id===id);if(!record)throw new Error("API 提供商不存在");const now=new Date().toISOString();if(kind==="connection"){record.lastTestStatus=status;record.lastTestAt=now;record.lastError=error}else{record.lastImageTestStatus=status;record.lastImageTestAt=now;record.lastImageTestError=error}record.updatedAt=now;return publicProvider(record)})}

export const SYC_DEFAULTS={name:"默认",baseUrl:"https://sycagent.top/v1",imageModel:"gpt-image-2",chatModel:"mix-gpt-5.4",stream:false,partialImages:1,returnBase64:true,codexCliCompatible:false,timeoutSeconds:600,enabled:true} as const;

function sycPublic(record?:ApiProviderSecretRecord):SycConfigPublic{
  const hasKey=Boolean(record?.encryptedApiKey),hasModel=Boolean(record?.imageModel||record?.defaultModel);
  return {
    ...
    apiKeyMask:record?.apiKeyMasked?`••••••••${record.apiKeyMasked.slice(-4)}`:"",
    imageModel:record?.imageModel||record?.defaultModel||SYC_DEFAULTS.imageModel,
    ...
    codexCliCompatible:record?.codexCliCompatible??SYC_DEFAULTS.codexCliCompatible,
    timeoutSeconds:record?.timeoutSeconds??SYC_DEFAULTS.timeoutSeconds,
    enabled:record?.enabled??SYC_DEFAULTS.enabled,
    lastTestStatus:record?.lastTestStatus||(hasKey&&hasModel?"untested":"incomplete"),
...
export async function getSycConfig(){return sycPublic((await loadStore()).apiProviders.find(item=>item.type==="syc-openai-compatible"))}

function validateSycInput(input:SycConfigInput,requireKey:boolean){
  if(!input.name.trim())throw new Error("配置名称不能为空");
  if(requireKey&&!input.apiKey?.trim())throw new Error("SYC 授权码 / API Key 不能为空");
  if(!input.imageModel.trim())throw new Error("图片模型不能为空");
  if(input.partialImages<0||input.partialImages>3)throw new Error("中间步骤图像数必须在 0 到 3 之间");
  if(input.timeoutSeconds<10||input.timeoutSeconds>900)throw new Error("请求超时必须在 10 到 900 秒之间");
  return {...input,name:input.name.trim(),baseUrl:normalizeSycBaseUrl(input.baseUrl),imageModel:input.imageModel.trim(),chatModel:input.chatModel?.trim()||"",apiKey:input.apiKey?.trim()};
}

export async function saveSycConfig(input:SycConfigInput){
  return mutate(async store=>{
    let record=store.apiProviders.find(item=>item.type==="syc-openai-compatible");
    const value=validateSycInput(input,!record?.encryptedApiKey);
    const now=new Date().toISOString(),keyChanged=Boolean(value.apiKey),connectionChanged=!record||record.baseUrl!==value.baseUrl||keyChanged||record.imageModel!==value.imageModel;
    if(!record){
      const key=value.apiKey!;
      record={id:crypto.randomUUID(),name:value.name,type:"syc-openai-compatible",baseUrl:value.baseUrl,encryptedApiKey:await encrypt(key),apiKeyMasked:maskKey(key),defaultModel:value.imageModel,enabled:value.enabled,notes:"SYC 中转站专用配置",lastTestStatus:"untested",createdAt:now,updatedAt:now};
      store.apiProviders.push(record);
    }else{
      record.name=value.name;record.baseUrl=value.baseUrl;record.defaultModel=value.imageModel;record.enabled=value.enabled;
      if(value.apiKey){record.encryptedApiKey=await encrypt(value.apiKey);record.apiKeyMasked=maskKey(value.apiKey)}
      record.updatedAt=now;
    }
    record.imageModel=value.imageModel;record.chatModel=value.chatModel;record.stream=value.stream;record.partialImages=value.partialImages;record.returnBase64=value.returnBase64;record.codexCliCompatible=value.codexCliCompatible;record.timeoutSeconds=value.timeoutSeconds;
    if(connectionChanged){record.lastTestStatus="untested";record.lastTestAt=undefined;record.lastError=undefined;record.lastImageTestStatus="untested";record.lastImageTestAt=undefined;record.lastImageTestError=undefined;record.lastTestLatencyMs=undefined}
    if(!record.enabled)for(const workflow of ["tryon","pose","recolor"] as const){if(store.workflowModelBindings[workflow].primary?.providerId===record.id)delete store.workflowModelBindings[workflow].primary;if(store.workflowModelBindings[workflow].fallback?.providerId===record.id)delete store.workflowModelBindings[workflow].fallback}
    return sycPublic(record);
  });
}

export async function deleteSycConfig(){const current=await getSycConfig();if(current.id==="syc-default")return false;return deleteApiProvider(current.id)}

export async function getSycRuntime(draft?:{baseUrl?:string;apiKey?:string;imageModel?:string}){
  const record=(await loadStore()).apiProviders.find(item=>item.type==="syc-openai-compatible");
  const baseUrl=normalizeSycBaseUrl(draft?.baseUrl||record?.baseUrl||SYC_DEFAULTS.baseUrl);
  const apiKey=draft?.apiKey?.trim()||(record?.encryptedApiKey?await decrypt(record.encryptedApiKey):"");
  if(!apiKey)throw new Error("SYC 授权码 / API Key 尚未配置");
  return {
    id:record?.id||"syc-default",name:record?.name||SYC_DEFAULTS.name,type:"syc-openai-compatible" as const,baseUrl,apiKey,
    model:draft?.imageModel?.trim()||record?.imageModel||record?.defaultModel||SYC_DEFAULTS.imageModel,source:"stored" as const,
    syc:{stream:record?.stream??SYC_DEFAULTS.stream,partialImages:record?.partialImages??SYC_DEFAULTS.partialImages,returnBase64:record?.returnBase64??SYC_DEFAULTS.returnBase64,codexCliCompatible:record?.codexCliCompatible??SYC_DEFAULTS.codexCliCompatible,timeoutSeconds:record?.timeoutSeconds??SYC_DEFAULTS.timeoutSeconds},
  };
}

export async function updateSycTestResult(kind:"connection"|"image",status:SycTestStatus,error?:string,latencyMs?:number){
  return mutate(store=>{
    const record=store.apiProviders.find(item=>item.type==="syc-openai-compatible");if(!record)throw new Error("请先保存 SYC 配置");
    const now=new Date().toISOString();
    if(kind==="connection"){record.lastTestStatus=status as ProviderTestStatus;record.lastTestAt=now;record.lastError=error;record.lastTestLatencyMs=latencyMs}
    else{record.lastImageTestStatus=status;record.lastImageTestAt=now;record.lastImageTestError=error}
    record.updatedAt=now;return sycPublic(record);
  });
}
