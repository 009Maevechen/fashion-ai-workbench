...
import {buildOpenAiCompatibleRequest,parseOpenAiCompatibleImage,providerInputImageFilename,type OpenAiCompatibleResponse} from "../provider-contracts";
...
export class OpenAiCompatibleProvider implements ImageProvider{
  ...
  async generate(input:GenerateInput,model:string):Promise<ProviderResult>{
    ...
    if(useMultipart){
      ...
      input.images.forEach((image,index)=>{const part=dataUrlParts(image),extension=part.mime.includes("png")?"png":part.mime.includes("webp")?"webp":"jpg";form.append("image",new Blob([part.bytes],{type:part.mime}),providerInputImageFilename(input.workflow,index,extension,input.images.length))});
