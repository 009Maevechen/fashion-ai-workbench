import {faceVisibilityPrompt} from "./face-visibility";

export const POSE_PROMPT_VERSION="pose-v3-strict-reference";
export const POSES=["自然站立，身体正面或轻微侧向镜头，双手自然放置，完整清晰地展示服装。","模特轻微迈步或自然转移重心，动作自然，不遮挡服装重点。","身体轻微侧转，一只手自然弯曲，展示服装侧面、袖型、腰线、裙型或裤型。"];
export function posePrompt(pose:string,productType:string,shot:string,face:boolean,background:boolean,details:string){return `任务：根据两张输入图生成一张独立的电商姿势图。
第一张输入图是商品模特图，决定模特身份、脸部、服装商品、颜色、结构、材质、背景与光线。
第二张输入图是强制姿势模板。必须逐项、严格复刻第二张图的姿势和构图，不得自行优化、自然化或改编。

姿势模板拥有最高优先级：
1. 严格复制头部角度、视线方向、肩线倾斜、躯干朝向、俯仰角度和扭转角度。
2. 严格复制左右手臂、肘部、手腕和手掌的位置与弯曲角度；手是否扶腰、下垂、抬起或放入口袋必须一致。
3. 严格复制骨盆方向、左右腿位置、膝盖和脚踝角度、站姿宽度、迈步方向与身体重心。
4. 严格复制人物在画面中的大小、位置、景别、裁切范围、镜头方向和留白比例。
5. 不得左右镜像，不得交换左右手或左右腿，不得增加、删除或移动肢体，不得改变站立或坐姿状态。
6. 如果文字姿势描述与第二张参考图有任何冲突，以第二张参考图为唯一标准。
7. 第二张图只提供人体姿势骨架与构图，不得复制其中的人物身份、脸、发型、服装、配饰、道具或背景。

必须保持第一张图中的同一个模特、同一套服装、颜色版型长度面料、全部服装细节、背景光线拍摄风格、正常人体比例、3:4竖版；每次只输出一张独立图片。
文字说明（仅用于辅助理解参考图，不得覆盖参考图）：${pose}
商品类型：${productType}
景别：${shot}
${faceVisibilityPrompt(face)}
背景要求：${background?"严格保持第一张图背景":"背景可自然调整，但不得影响姿势复刻"}
细节保护：${details}
禁止更换模特、改变服装设计或颜色、增删细节、修改参考姿势、拼图多宫格、文字水印边框、额外肢体或畸形、直接返回任一输入图。`}
