...
export const TRYON_PROMPT_VERSION="tryon-v8-model-subject-priority";
...
两张输入图的职责（严格区分，不可混淆）：
- 服装产品图：这是生成结果中服装外观的唯一真值、唯一来源和最高优先级依据。无论模型或 API 如何排列输入图片，都必须按“服装产品图 / garment / product image”的语义识别它，不得按图片序号混淆职责。服装产品图只负责提供服装外观（款式、颜色、面料、细节、版型），除此之外的一切（人物、姿势、景别、构图、场景、背景、光线、展示方式）一律不参考、不采用、不得成为画面主体。
