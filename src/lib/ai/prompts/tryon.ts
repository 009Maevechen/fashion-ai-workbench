...
export const TRYON_PROMPT_VERSION="tryon-v3-product-copy";
...
人物参考图只提供模特的身份、姿势、身体比例、景别、背景和光线。服装参考图提供需要换上的真实服装商品。
核心规则：把服装参考图中的产品服装一比一复刻，真实地穿到人物参考图中的模特身上。只替换模特身上的服装，只参考模特图的姿势、身份、背景和光线，不采用模特原来穿着的服装的任何设计，也不把模特原服装套进产品中。
...
禁止：更换模特；改变服装长度；增加或删除结构装饰；保留或混入模特原服装的款式、颜色、图案或细节；直接返回原模特图；生成多宫格、对比图、文字或水印；每次只输出一张独立换装图。
