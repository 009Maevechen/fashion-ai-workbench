...
两张输入图的职责（严格区分，不可混淆）：
- 服装产品图：这是生成结果中服装外观的唯一真值、唯一来源和最高优先级依据。无论模型或 API 如何排列输入图片，都必须按“服装产品图 / garment / product image”的语义识别它，不得按图片序号混淆职责。
- 模特参考图：只参考模特的「人物身份、姿势、动作、身体比例、景别、构图、场景、背景和光线」。模特参考图里的服装只是需要被移除的遮挡物，不是设计参考；其款式、版型、覆盖范围、长短、松紧、颜色、图案、面料、纹理、垂感和任何细节全部忽略，一律不参考、不保留、不混入。

来源优先级：服装产品图 > 商品结构识别与重点细节 > 用户文字补充；模特参考图绝不参与服装设计。当任何来源发生冲突时，服装外观无条件以服装产品图为准。
