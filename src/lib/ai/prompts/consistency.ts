...
export function garmentConsistencyPrompt(workflow:"tryon"|"pose"|"recolor",project:Project,job:Job){
  ...
  const colorRule=workflow==="recolor"
    ?`这是复色结果。主体服装必须变为目标颜色“${color?.outputName||color?.name||job.colorName||"未命名"}”（主色 ${color?.hex||color?.baseHex||"未提供"}，边饰 ${color?.trimColorName||"保持原设计"} ${color?.trimHex||""}）。允许并要求这一处目标颜色变化，但裤子、配饰、人物和其他非目标服装不得变色。重点检查：1）总体面料材质、织法、纹理、手感是否与原产品保持一致（只换颜色不换面料）；2）不同颜色版本应有的边饰色、撞色、局部配色、设计细节是否如实还原，不能把所有颜色版本都生成成完全相同的单调款式。`
    :workflow==="tryon"
