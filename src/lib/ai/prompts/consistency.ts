...
export function garmentConsistencyPrompt(
  ...
  const colorRule =
    workflow === "recolor"
      ...
      : workflow === "tryon"
        ? `这是服装换装候选图。必须与产品服装和各局部特写一比一复刻，服装类型、版型、材质、面料、纹理、颜色、包边和全部设计细节完全一致；模特、姿势、构图和背景差异不算服装不一致。若产品依据中同时出现多件或多个颜色款，结果只能对应其中同一件、同一颜色款，颜色和全部细节必须来自这一件，不能跨款拼接。结构化细节锁：${lockedFields}。重点逐项检查：1）singleSourceGarment：是否只采用同一件、同一颜色款，且没有混入另一件的颜色、图案、面料或细节；2）是否混入参考模特原服装；3）商品类别、商品子类、版型、廓形、长度、领口、袖型、门襟和下摆；4）扣子数量、相对位置、大小、颜色、形状、排列和门襟方向；5）口袋数量、位置、开口与形状；6）条纹数量与位置、包边、车线的数量、方向和位置；7）印花、刺绣、拼接的形状、比例和位置；8）面料材质、织法、纹理、厚薄、光泽与垂感；9）左右结构是否一致；10）是否新增产品图不存在的设计；11）是否丢失产品图已有设计；12）是否有错位、畸形、杂质、重复或明显AI异常。任何混款或可见细节错误都必须 consistent=false。若错误只局限于扣子、口袋、条纹、包边、车线、印花、刺绣、拼接、领口、袖口、下摆或面料局部，必须在 repairTargets 中给出错误类型、具体修复说明、该错误在最后一张输出图中的归一化 boundingBox（x/y/width/height 均为0到1）和置信度；无法可靠定位则不要伪造 boundingBox。`
