...
export function garmentConsistencyPrompt(
  ...
  const hasIndependentColorReferences =
    referenceEvidence.mode === "independent";
  const hasColorReference = referenceEvidence.images.length > 0;
  ...
  const lockedFields = detailLock
    ? Object.entries(detailLock.fields)
        ...
        .join("；")
    : "未建立结构化细节锁";
  const recolorReferenceCount = referenceEvidence.images.length;
  const recolorResultIndex = recolorReferenceCount + 2;
  const recolorReferenceRange =
    recolorReferenceCount === 0
      ? "无颜色参考图"
  ...
  const colorRule =
    workflow === "recolor"
      ? `这是“同款不同色”复色结果。商品类型为${project.productType}。第1张已确认姿势图是人物、姿势、动作、景别、构图、背景、画面样式、服装版型、结构、面料、纹理、垂感、扣子五金和全部颜色区域布局的唯一底图；${hasColorReference ? `${recolorReferenceRange}是当前颜色款参考图，${selectedMainColor ? "只负责提供明确局部配色及其位置，主体色由人工基本色锁定" : complexColorway ? "负责提供主体及明确复杂分区的真实颜色" : "只负责提供普通颜色款的主体色"}；` : "当前没有颜色参考图，仅允许按人工基本色检查简单单色复色；"}第${recolorResultIndex}张是待检结果。${hasIndependentColorReferences ? `当前只有${recolorReferenceRange}这一张生效中的新独立参考图；检查时必须把它作为唯一颜色标准，完全忽略历史独立图、SKU 级多色图、其他颜色款、旧共享裁图及旧结果。` : hasColorReference ? `当前没有独立参考图，${recolorReferenceRange}来自共享颜色参考图中的当前色款；允许按其真实可见的主体与局部配色复色，但不能据此改款。` : "当前没有任何图片颜色依据；仅允许简单单色款按人工基本色复色，复杂配色不得猜测。"}默认规则是只改颜色、不改款式。当前颜色款没有获准改变结构，口袋、条纹、拼接、包边、扣子、印花、车线、下摆、袖口、门襟和领口必须与第1张完全一致。${selectedMainColor || !hasColorReference ? `人工选定基本色“${referenceEvidence.promptColorName}”${referenceEvidence.promptHex ? `（${referenceEvidence.promptHex}）` : ""}是主体色唯一验收依据。` : `颜色名称和旧 HEX 只作标签；颜色判断必须服从本次实际传入的当前色款参考图。`}颜色映射：${colorMapText || variantColors}。${colorMapText ? `逐部位颜色验收：必须核对主体身片、罗纹、条纹和拼接是否与映射一一对应；任何关键部位颜色不一致都必须 colorMapping=false、consistent=false。` : ""}${complexColorway ? `复杂款参考观察仅用于颜色区域定位、不能授权改款：${variantDetails}。参考图面料观察：${variantMaterial}。` : "普通款不要求逐项分析边饰或五金颜色。"}实际面料类别、织法、纹理、厚薄、光泽、垂感以及扣子/五金颜色仍以第1张为准。参考图即使斜拍、悬挂、折叠或透视变形，也必须按第1张原款布局核对对应色区，不得把参考图角度、褶皱、裁切或位置偏移带入结果。${occlusionRule}必须同时检查：1）最后1张的人物、露脸状态、动作、景别、构图和背景是否与第1张一致；2）最后1张是否仍是第1张的同一个款式；3）主体色是否匹配${hasIndependentColorReferences ? "当前唯一新参考图" : hasColorReference ? "当前共享参考图中的该色款" : "人工基本色"}，允许的局部颜色是否匹配对应参考区域；4）颜色是否越界；5）面料纹理、织法、光泽、厚薄、垂感是否与第1张一致；6）拉链与拉头、扣子/按扣、铆钉、搭扣、腰袢、抽绳、褶裥、品牌标、刺绣、印字、明线和装饰车线的数量、材质、表面色、尺寸与位置是否直接继承第1张；7）领口/领座/翻领的高度、宽度、翻折、领尖、驳头与缺口是否不变；8）背景、皮肤、头发、鞋子、配饰、道具等非目标区域是否完全未改；9）服装与整张画面是否干净，没有新增灰雾、灰色薄膜、脏斑、尘点、煤灰感、霉斑感、随机颗粒、斑驳噪声、涂抹或材质糊化；10）曝光、白平衡、对比度、黑位、肤色与背景光影是否保持第1张，不存在全图压暗、去饱和、蒙灰或重新调色。任何微细节被简化、增删、移动或重画时 details=false、construction=false、consistent=false。cleanliness 只有在没有任何新增脏污或灰雾时才为 true；toneIntegrity 只有在整图色调与第1张一致时才为 true；artifactFree 只有在没有模糊、涂抹、异常噪点、断裂纹理和明显 AI 杂质时才为 true。即使目标色是灰色，也必须是干净、明确、有层次的目标灰，不能把灰色污染误判为正确颜色。任何款式变化、颜色错位、历史颜色混入、非服装区域误改、无依据补全遮挡区、人物动作或构图改变、灰雾脏感、全局色调变化或材质糊化，consistent 必须为 false。`
      : workflow === "tryon"
  ...
  const imageOrder =
    workflow === "recolor"
      ? `图片顺序：第1张是上一流程已确认姿势图和原款结构布局基准，${hasColorReference ? `${recolorReferenceRange}是当前唯一生效颜色参考图，` : "没有颜色参考图，"}第${recolorResultIndex}张是复色结果图。`
      : workflow === "tryon"
