...
复色成功硬性标准：第一张图只允许在目标服装区域内发生变化，目标颜色必须准确统一；不得保留原主体颜色；没有真正改变目标服装主体颜色必须视为生成失败。先锁定第一张图的人物和画面，再将第二张图当前色款明确可见的设计映射到相同服装部位。第二张图有独特口袋、条纹、拼接、扣子、印花、包边、车线、线条位置或面料分区时必须同步还原；第二张图没有的设计不得沿用其他色款。面料类别、织法/针法、纹理清晰度、厚薄、光泽、垂感和褶皱响应必须前后一致，不得因改色变成另一种材质。不得猜测、混入其他颜色款细节或把复色做成无依据的新衣服；渐变方向、层次和过渡边界必须保持对应色款真实设计。
