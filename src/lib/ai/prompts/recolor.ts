...
export function recolorPrompt(area:string,color:string,hex:string,protectedAreas:string[],extra:string,showFace=false){return `任务：服装商品精准复色。第一张图片是需要修改颜色的模特商品图。如果有第二张图片，它只是目标颜色样本。只提取颜色本身，严禁从参考图复制服装款式、版型、长度、领口、袖口、纽扣、图案、印花、材质、纹理或任何结构。只修改第一张图片中指定服装区域的主体面料颜色。
目标服装区域：${area}\n目标颜色名称：${color}\n目标HEX色值：${hex||"未指定"}
