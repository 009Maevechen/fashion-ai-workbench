...
export async function storageStats(){const trash=await dirSize(path.join(root,".trash"));return {outputsBytes:Math.max(0,(await dirSize(root))-trash),trashBytes:trash,outputPath:root}}
