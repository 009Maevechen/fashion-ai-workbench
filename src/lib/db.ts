...
export type ProductDetailAssetKey="productFrontImage"|"productBackImage"|"productDetailImage"|"printCloseupImage"|"buttonCloseupImage"|"brandTagImage"|"modelReferenceImage"|"fabricTextureImage"|"colorReferenceImage";
export type NormalizedCropRegion={x:number;y:number;width:number;height:number};
export type ProductDetailRegionType="frontView"|"backView"|"detail"|"print"|"buttons"|"label"|"fabric"|"multiColor"|"modelReference";
...
export type JobPhase="queued"|"uploading"|"submitting"|"waiting_provider"|"downloading"|"validating"|"saving"|"success"|"failed"|"interrupted";
...
export const createProject=(v:Pick<Project,"sku"|"productName"|"productType">)=>mutate(s=>{if(s.projects.some(x=>x.sku===v.sku))throw new Error("SKU 已存在");const now=new Date().toISOString();const p:Project={...v,id:crypto.randomUUID(),currentStep:1,status:"未开始",createdAt:now,updatedAt:now,assets:{},settings:{},stepStatuses:{"1":"ready","2":"not_started","3":"not_started","4":"not_started","5":"not_started"},dependencyStatus:"current",targetColors:[]};s.projects.push(p);return p});
