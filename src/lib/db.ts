...
export type PoseShotType =
  "full_body" | "half_body" | "upper_body" | "lower_body";
export type PoseFaceMode = "visible" | "hidden" | "either";
export type PoseReferenceInput = {
  id: string;
  poseIndex: 1 | 2 | 3;
  imagePath: string;
...
export type PoseTemplateGroup = {
  ...
  lastUsedAt?: string;
  sourceProjectId?: string;
  poses: [PoseTemplateItem, PoseTemplateItem, PoseTemplateItem];
};
...
export type ProjectAssets = {
  ...
  printCloseupImage?: string;
  buttonCloseupImage?: string;
  pocketCloseupImage?: string;
  necklineCloseupImage?: string;
  sleeveCloseupImage?: string;
  hemCloseupImage?: string;
  stitchingCloseupImage?: string;
  modelReferenceImage?: string;
  fabricTextureImage?: string;
  otherMaterialImages?: string[];
  modelImage?: string;
  standalonePoseInputImage?: string;
  poseReferenceImages?: string[];
  colorReferenceImage?: string;
  colorReferenceCropImage?: string;
  colorReferenceCropRegion?: {
    x: number;
    y: number;
    width: number;
    height: number;
...
export type ProductDetailAssetKey =
  | "productFrontImage"
  | "productBackImage"
  | "productDetailImage"
  | "printCloseupImage"
...
export type ProductDetailRegionType =
  | "frontView"
  | "backView"
  | "detail"
  | "print"
  ...
  | "multiColor"
  | "modelReference";
export type ProductVisualRegion = {
  id: string;
...
export type ProductAssetEvidence = {
  source: "manual" | "manual_crop" | "ai_crop";
  sourceImage?: string;
  confidence?: number;
  boundingBox?: NormalizedCropRegion;
  needsReview?: boolean;
...
export type ProductVisualAnalysis = {
  ...
  analyzedAt?: string;
  error?: string;
  regions: ProductVisualRegion[];
  missing: ProductMissingDetail[];
  attributeEvidence?: Partial<
    Record<keyof ProductAttributes, ProductAttributeEvidence>
  >;
  aiFilledAttributeKeys?: Array<keyof ProductAttributes>;
  suggestedProtectionItems?: string[];
};
export type GarmentDetailLockFieldKey =
  | "category"
  | "subcategory"
  | "fit"
  | "silhouette"
  ...
  | "symmetry"
  | "otherDetails";
export type GarmentDetailVisibility =
  "visible" | "not_visible" | "not_applicable";
export type GarmentDetailLockValue = {
  value: string;
  confidence: number;
  visibility: GarmentDetailVisibility;
  sourceRoles: string[];
};
export type GarmentDetailReference = {
  role: string;
  label: string;
  image: string;
  priority: number;
};
export type GarmentDetailLock = {
  version: "garment-detail-lock-v2";
  status: "locked" | "needs_review";
  sourceImage: string;
  sourceSignature: string;
  productType: ProductType;
  fields: Record<GarmentDetailLockFieldKey, GarmentDetailLockValue>;
...
export type ProductProfile = {
  ...
  deliveryDate?: string;
  reviewStatus?: "draft" | "awaiting_review" | "confirmed";
  notes?: string;
  detailDescription?: string;
  tags?: string[];
  protectionItems?: string[];
  attributes?: ProductAttributes;
  manualAttributeKeys?: Array<keyof ProductAttributes>;
  manualProtectionEdited?: boolean;
};
export type PoseReferenceAnalysis = {
...
export type VariantColorRegion = {
  part: string;
  colorName: string;
  hex?: string;
  confidence: number;
...
export type TryonRevisionMessage = {
  id: string;
  role: "user" | "assistant";
  content: string;
  createdAt: string;
...
export type WorkflowSettings = {
  workflowSkus?: Partial<Record<WorkflowType, string>>;
  tryon?: {
    mode: GenerationMode;
    ...
    revisionMessages?: TryonRevisionMessage[];
  };
  pose?: {
    mode: GenerationMode;
    productType?: ProductType;
    ...
    referenceAnalyses?: PoseReferenceAnalysis[];
    sourceMode: "confirmed" | "standalone";
    referenceMode?: "library" | "upload";
    selectedResultImages?: string[];
    focus?: "" | "upper" | "lower";
  };
  recolor?: {
    mode: GenerationMode;
...
export type Project = {
  ...
  poseReferenceInputs?: PoseReferenceInput[];
...
export type JobPhase =
  ...
  | "failed"
  | "interrupted";
export type GarmentConsistencyCheck = {
  status: "passed" | "needs_review" | "needs_redo" | "failed";
  score?: number;
  summary: string;
  issues: string[];
...
export type CorrectionInstructionCheck = {
  passed: boolean;
  score: number;
  summary: string;
  missed: string[];
...
export type Job = {
  ...
  provider: string;
  providerId?: string;
  providerType?: ApiProviderType | string;
  model: string;
  modelSlot?: ModelSlot;
  mode: GenerationMode;
  inputImages: string[];
  ...
  tryonConsistency?: TryOnConsistencyReport;
  detailRepairOfJobId?: string;
  detailRepairAttempt?: number;
  consistencyCheck?: GarmentConsistencyCheck;
...
export type Store = {
  ...
  poseTemplateGroups: PoseTemplateGroup[];
};

const dataDir = runtimeDataDir(),
  file = path.join(dataDir, "store.json"),
  legacyBackup = path.join(dataDir, "store.backup.json"),
  backups = [1, 2, 3].map((index) =>
    path.join(dataDir, `store.backup-${index}.json`),
  );
...
const normalizeStore = (parsed: Partial<Store>): Store => ({
  ...
  poseTemplateGroups: parsed.poseTemplateGroups || [],
});
...
const isStore = (value: unknown): value is Store => {
  const item = value as Partial<Store> | null;
  return Boolean(
    item &&
    Array.isArray(item.projects) &&
    Array.isArray(item.jobs) &&
    Array.isArray(item.operations) &&
    Array.isArray(item.poseTemplateGroups),
  );
};
type PersistenceRuntime = typeof globalThis & {
  __workbenchRecoveredBackup?: string;
  __workbenchPersistenceError?: string;
};
const persistenceRuntime = globalThis as PersistenceRuntime;
async function load(): Promise<Store> {
  try {
    const loaded = await readJsonWithBackups(
      file,
    ...
    if (loaded.source !== file) {
      persistenceRuntime.__workbenchRecoveredBackup = path.basename(
        loaded.source,
      );
      await durableWriteJson(file, loaded.value);
    }
    return normalizeStore(loaded.value);
  } catch (error) {
    const message = error instanceof Error ? error.message : "未知数据错误";
    persistenceRuntime.__workbenchPersistenceError = message;
    throw new Error(`项目数据损坏且所有备份均不可用：${message}`);
  }
}
...
async function persistProjectProcessFiles(store: Store, projectIds?: string[]) {
  ...
  const selected = projectIds
    ? store.projects.filter((project) => projectIds.includes(project.id))
    : store.projects;
  for (const project of selected) {
    const directory = path.resolve(root, safeSegment(project.sku));
    if (!directory.startsWith(root + path.sep))
      throw new Error("非法商品流程目录");
    await fs.mkdir(directory, { recursive: true });
    const target = path.join(directory, "project-process.json"),
      temporary = `${target}.${process.pid}.${crypto.randomUUID()}.tmp`;
    const snapshot = {
      schemaVersion: SCHEMA_VERSION,
      savedAt: new Date().toISOString(),
      project,
      jobs: store.jobs.filter((job) => job.projectId === project.id),
      operations: store.operations.filter(
        (operation) => operation.projectId === project.id,
      ),
    };
    try {
      await durableWriteJson(temporary, snapshot);
      await replaceStoreFile(temporary, target);
    } finally {
...
async function rotateBackups() {
  await fs.copyFile(backups[1], backups[2]).catch(() => {});
  await fs.copyFile(backups[0], backups[1]).catch(() => {});
  try {
    const current = await readValidJson(file, isStore);
    await durableWriteJson(backups[0], current);
  } catch {}
}
async function mutate<T>(
  fn: (s: Store) => T | Promise<T>,
  affectedProjectIds?: string[] | (() => string[]),
) {
  let result!: T;
  queue = queue.then(async () => {
    ...
    await rotateBackups();
    await durableWriteJson(file, s);
    const affected =
      typeof affectedProjectIds === "function"
        ? affectedProjectIds()
        : affectedProjectIds;
    await persistProjectProcessFiles(s, affected);
  });
  await queue;
  return result;
}
export function persistenceStatus() {
  return {
    recovered: Boolean(
      persistenceRuntime.__workbenchRecoveredBackup ||
      process.env.AI_STUDIO_RECOVERED === "1",
    ),
    backup: persistenceRuntime.__workbenchRecoveredBackup,
    error: persistenceRuntime.__workbenchPersistenceError,
  };
}
const normalize = (p: Project): Project => ({
  ...p,
  assets: p.assets || {},
  profile: p.profile || { reviewStatus: "draft", tags: [], attributes: {} },
  settings: p.settings || {},
  ...
  dependencyStatus: p.dependencyStatus || "current",
  targetColors: p.targetColors || [],
  poseReferenceInputs: p.poseReferenceInputs || [],
  poseReviewStates: p.poseReviewStates || {},
});
...
export const getProject = async (idOrSku: string) => {
  const p = findProjectByRouteKey((await load()).projects, idOrSku);
  return p ? normalize(p) : undefined;
};
export const createProject = (
  v: Pick<Project, "sku" | "productName" | "productType">,
) =>
  mutate((s) => {
    const sku = v.sku.trim();
    if (!sku) throw new Error("SKU 不能为空");
    const normalized = sku.toLocaleLowerCase();
    const duplicate = s.projects.find(
      (x) => x.sku.trim().toLocaleLowerCase() === normalized,
    );
    if (duplicate)
      throw new Error(
        `SKU「${duplicate.sku}」已存在（商品名称：${duplicate.productName || "未命名"}），请换一个货号`,
      );
...
export const updateProjectWith = (
  id: string,
  ...
  mutate(
    (s) => {
      const p = s.projects.find((x) => x.id === id);
      if (!p) throw new Error("商品项目不存在");
      const patch = updater(normalize(p));
      Object.assign(p, patch, {
        id: p.id,
        updatedAt: new Date().toISOString(),
      });
...
export const deleteProject = (id: string) =>
  mutate((s) => {
    ...
    s.jobs = s.jobs.filter((x) => x.projectId !== id);
    s.operations = s.operations.filter((x) => x.projectId !== id);
  });
export const listJobs = async (filters?: {
  ...
  let j = (await load()).jobs;
  if (filters?.sku) j = j.filter((x) => x.sku.includes(filters.sku!));
  if (filters?.workflow) j = j.filter((x) => x.workflow === filters.workflow);
  if (filters?.projectId)
    j = j.filter((x) => x.projectId === filters.projectId);
  return j.sort((a, b) => b.startedAt.localeCompare(a.startedAt));
};
...
export const patchJob = (id: string, patch: Partial<Job>) => {
  let projectId = "";
  return mutate(
    (s) => {
...
export const getJob = async (id: string) =>
  (await load()).jobs.find((x) => x.id === id);
export const deleteWorkflowRecords = (
  ...
  mutate((s) => {
    const jobIds = new Set(
      s.jobs
        .filter(
          (job) => job.projectId === projectId && job.workflow === workflow,
        )
        .map((job) => job.id),
    );
    s.jobs = s.jobs.filter((job) => !jobIds.has(job.id));
    s.operations = s.operations.filter(
      (operation) =>
        !(operation.projectId === projectId && operation.workflow === workflow),
    );
    return jobIds.size;
  });
export const markInterruptedJobs = () =>
  mutate((s) => {
    const count = interruptJobs(s.jobs);
    if (count)
      persistenceRuntime.__workbenchRecoveredBackup =
        persistenceRuntime.__workbenchRecoveredBackup || "unfinished-tasks";
    return count;
  });
...
export const getOperation = async (id: string) =>
  (await load()).operations.find((x) => x.id === id);
export const patchOperation = (id: string, patch: Partial<Operation>) => {
  let projectId = "";
  return mutate(
    (s) => {
      const operation = s.operations.find((x) => x.id === id);
      if (!operation) throw new Error("本地任务不存在");
...
export const listOperations = async (projectId?: string) => {
  const operations = (await load()).operations;
  return (
    projectId ? operations.filter((x) => x.projectId === projectId) : operations
  ).sort((a, b) => b.createdAt.localeCompare(a.createdAt));
};
export const markInterruptedOperations = () =>
  mutate((s) => {
    const count = interruptOperations(s.operations);
    if (count)
      persistenceRuntime.__workbenchRecoveredBackup =
        persistenceRuntime.__workbenchRecoveredBackup || "unfinished-tasks";
    return count;
  });
export const reconcileGeneratingProjects = () =>
  mutate((s) => reconcileProjects(s.projects, s.operations));
// 暂停某个项目工作流的生图：把排队中/运行中的操作与未完成任务标记为中断，并对账项目状态。
export const cancelWorkflowGeneration = (
  projectId: string,
  workflow: WorkflowType,
) =>
  mutate((s) => {
    ...
    for (const job of s.jobs)
      if (
        ...
        job.phase &&
        !["success", "failed", "interrupted"].includes(job.phase)
      ) {
        job.phase = "interrupted";
        job.status = "interrupted";
        job.requestStatus = "interrupted";
        job.error = "任务已被用户取消";
        job.errorMessage = "任务已被用户取消";
        job.finishedAt = now;
        jobs++;
      }
    reconcileProjects(s.projects, s.operations, now);
    return { jobs, operations };
  });
// 清空某个项目工作流的错误生图内容：只删除失败/中断的任务，保留成功结果与上传素材。
export const clearWorkflowErrors = (
  projectId: string,
  workflow: WorkflowType,
) =>
  mutate((s) => {
    const ids = s.jobs
      ...
      .map((job) => job.id);
    s.jobs = s.jobs.filter((job) => !ids.includes(job.id));
    return ids.length;
  });
export const listPoseTemplateGroups = async () =>
  [...(await load()).poseTemplateGroups].sort((a, b) =>
    (b.lastUsedAt || b.updatedAt).localeCompare(a.lastUsedAt || a.updatedAt),
  );
export const getPoseTemplateGroup = async (id: string) =>
  (await load()).poseTemplateGroups.find((group) => group.id === id);
export const addPoseTemplateGroup = (group: PoseTemplateGroup) =>
  mutate((s) => {
    if (s.poseTemplateGroups.some((item) => item.id === group.id))
      throw new Error("姿势模板组已存在");
    s.poseTemplateGroups.push(group);
    return group;
  });
export const patchPoseTemplateGroup = (
  id: string,
  patch: Partial<PoseTemplateGroup>,
) =>
  mutate((s) => {
    const group = s.poseTemplateGroups.find((item) => item.id === id);
    if (!group) throw new Error("姿势模板组不存在");
...
export const removePoseTemplateGroup = (id: string) =>
  mutate((s) => {
    const group = s.poseTemplateGroups.find((item) => item.id === id);
    if (!group) throw new Error("姿势模板组不存在");
    s.poseTemplateGroups = s.poseTemplateGroups.filter(
      (item) => item.id !== id,
    );
    return group;
  });
